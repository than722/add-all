...
// Using programsList for demonstration. In a real app, you'd fetch user's enrolled programs.
...
const MyPrograms: React.FC = () => {
  ...
  // In a real application, you would fetch the *actual* programs enrolled by the current user.
  const enrolledProgramsDemo = programsList.filter((_, idx) => idx % 2 === 0); // Just a demo subset

  const filteredPrograms = enrolledProgramsDemo.filter((p) =>
  ...
  return (
    <div>
      ...
      {/* Search input field, similar to AllPrograms */}
      ...
      <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
        {filteredPrograms.length > 0 ? (
          ...
          <div className="col-span-full text-center text-gray-500 text-lg p-8">
            No enrolled programs found matching your search.
          </div>
        )}
      </div>
    </div>
