...
import SendRemarksModal from '@/components/ui/Modals/InstructorModal/RemarksModal';
...
const StudentListClient = () => {
  ...
  const [remarksStudent, setRemarksStudent] = useState<PendingApplication | null>(null);
  ...
  const handleSendRemarks = (title: string, description: string) => {
    console.log(`Remarks for ${remarksStudent?.name}:`, { title, description });
    alert(`Remarks sent to ${remarksStudent?.name}:\n\nTitle: ${title}\nDescription: ${description}`);
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      {remarksStudent && (
        <SendRemarksModal
          isOpen={remarksStudent !== null}
          studentName={remarksStudent?.name || ''}
          onClose={() => setRemarksStudent(null)}
          onSubmit={handleSendRemarks}
        />
      )}
    </div>
  );
