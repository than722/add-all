...
interface DeleteModuleModalProps {
  isOpen: boolean;
  moduleName: string; // The name of the module to be deleted
  onConfirm: () => void; // Function to call when delete is confirmed
  onCancel: () => void; // Function to call when delete is cancelled
}

/**
 * DeleteModuleModal component provides a confirmation dialog for deleting a module.
 * It takes the module's name, and functions for confirmation and cancellation.
 * The design is consistent with ArchiveModal.tsx.
 */
const DeleteModuleModal: React.FC<DeleteModuleModalProps> = ({ isOpen, moduleName, onConfirm, onCancel }) => {
  ...
  return (
    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center px-2 sm:px-0">
      ...
      <div className="bg-white p-4 sm:p-6 rounded-lg shadow-xl w-full max-w-xs sm:max-w-sm">
        {/* Modal title */}
        <h3 className="text-base sm:text-lg font-bold mb-2 sm:mb-4 text-[#08228d]">Delete Module</h3>
        {/* Modal message explaining the action */}
        <p className="mb-4 sm:mb-6 text-gray-900 text-sm sm:text-base">
          Are you sure you want to delete the module <span className="font-semibold">{moduleName}</span> and all its subsections? This action cannot be undone.
        </p>
